...
# Version: 4.1 - Multi-Tunnel Support + Kharej-to-Kharej with Separate Naming
...
show_all_tunnels() {
    ...
    echo -e "\n${BLUE}━━━ Kharej-Kharej Tunnels (172.17.x.x) ━━━${NC}"
...
main_menu() {
    while true; do
        clear
        echo -e "${GREEN}╔════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║   GRE Tunnel Manager v4.1              ║${NC}"
        echo -e "${GREEN}║   Multi-Tunnel + Kharej-to-Kharej      ║${NC}"
        echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${BLUE}Iran Server Options:${NC}"
        echo "  1) Add Single Kharej Tunnel (iran-X)"
        echo "  2) Add Multiple Kharej Tunnels"
        echo ""
        echo -e "${BLUE}Kharej Server Options:${NC}"
        echo "  3) Configure as Kharej (receiving from Iran)"
        echo ""
        echo -e "${BLUE}Kharej-to-Kharej Options:${NC}"
        echo "  4) Configure Kharej1 (Forward to Kharej2) → kharej-X"
        echo "  5) Configure Kharej2 (Destination)"
        echo ""
        echo -e "${BLUE}Management:${NC}"
        echo "  6) Show All Tunnels Status"
        echo "  7) Remove Specific Tunnel"
        echo "  8) Remove All Tunnels"
        echo ""
        echo "  9) Exit"
        echo ""
        echo -e "${YELLOW}Tunnel Naming:${NC}"
        echo -e "  • Iran-Kharej: ${GREEN}iran-1, iran-2...${NC} (172.16.x.x)"
        echo -e "  • Kharej-Kharej: ${GREEN}kharej-1, kharej-2...${NC} (172.17.x.x)"
        echo ""
        ...
        case "$choice" in
            ...
            3)
                ...
                ;;
            4)
                echo -e "\n${YELLOW}Configuring Kharej1 (Forward to another Kharej)...${NC}\n"
                echo -e "${BLUE}This is the first Kharej that receives traffic from Iran${NC}"
                echo -e "${BLUE}and forwards it to a second Kharej server${NC}\n"
                
                read -p "Enter Kharej1 Server Public IPv4 (this server): " kharej1
                validate_ipv4 "$kharej1" || { read -p "Press Enter to continue..."; continue; }
                
                read -p "Enter Kharej2 Server Public IPv4 (destination): " kharej2
                validate_ipv4 "$kharej2" || { read -p "Press Enter to continue..."; continue; }
                
                echo -e "\n${YELLOW}Enter ports to forward to Kharej2 (comma-separated)${NC}"
                echo -e "${YELLOW}Example: 443,8443,2053${NC}"
                read -p "Ports: " ports
                validate_ports "$ports" || { read -p "Press Enter to continue..."; continue; }
                ...
                ;;
            5)
                echo -e "\n${YELLOW}Configuring Kharej2 (Destination Kharej)...${NC}\n"
                echo -e "${BLUE}This is the second Kharej that receives traffic from Kharej1${NC}\n"
                
                read -p "Enter Kharej2 Server Public IPv4 (this server): " kharej2
                validate_ipv4 "$kharej2" || { read -p "Press Enter to continue..."; continue; }
                
                read -p "Enter Kharej1 Server Public IPv4 (source): " kharej1
                validate_ipv4 "$kharej1" || { read -p "Press Enter to continue..."; continue; }
                
                echo -e "\n${YELLOW}What tunnel number did you use on Kharej1? (e.g., 1 for kharej-1)${NC}"
